-- Drop existing tables if they exist (cascade to remove dependent data)
DROP TABLE IF EXISTS lcl_template_history CASCADE;
DROP TABLE IF EXISTS lcl_template_items CASCADE;
DROP TABLE IF EXISTS lcl_template_structures CASCADE;

-- Create LCL Template Structures table
CREATE TABLE lcl_template_structures (
...
-- Create unique constraint on company_id and name
CREATE UNIQUE INDEX idx_lcl_template_structures_company_name ON lcl_template_structures(company_id, name) WHERE is_active = true;

-- Create LCL Template Items table
CREATE TABLE lcl_template_items (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  company_id UUID NOT NULL REFERENCES companies(id) ON DELETE CASCADE,
...
CREATE TABLE lcl_template_history (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  company_id UUID NOT NULL REFERENCES companies(id) ON DELETE CASCADE,
...
CREATE INDEX idx_lcl_template_structures_company_id ON lcl_template_structures(company_id);
CREATE INDEX idx_lcl_template_structures_is_active ON lcl_template_structures(is_active);
CREATE INDEX idx_lcl_template_items_structure_id ON lcl_template_items(structure_id);
CREATE INDEX idx_lcl_template_items_company_id ON lcl_template_items(company_id);
CREATE INDEX idx_lcl_template_items_section_subsection ON lcl_template_items(section_id, subsection_id);
CREATE INDEX idx_lcl_template_items_sort_order ON lcl_template_items(sort_order);
CREATE INDEX idx_lcl_template_history_company_id ON lcl_template_history(company_id);
